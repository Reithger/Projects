...
public class Entity {
	...
	public BufferedImage retrieveImage(String path) {
			try {
				if(path.charAt(path.length()-1) == '/') {
					path = (new File(InteractFrame.class.getClassLoader().getResource(path).toURI()).listFiles()[0]).getAbsolutePath();
				}
				else {
					path = InteractFrame.class.getClassLoader().getResource(path).toURI().getPath();
				}
	...
	public void setVisual(String fileLocation){
		...
		BufferedImage img = retrieveImage(fileLocation);
		width = img.getWidth();
