...
public class Social extends TimerTask{
	...
	public Social(SocializationScreen soc, BufferedImage mapIn, String filePath){
		parent = soc;
		...
		for(int i = 0; i < map.getWidth(); i++){
			for(int j = 0; j < map.getHeight(); j++){
				if(new Color(map.getRGB(i, j)).equals(new Color(178, 1, 25))){
					cha.add(new Character(j*4 - 35, i*4, filePath + "Assets/Entities/Female/", CORRELATE[cha.size()], TIER_LIST_NAMES[CORRELATE[cha.size()]], filePath));
				}
