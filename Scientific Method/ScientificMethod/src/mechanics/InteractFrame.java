...
public class InteractFrame extends JPanel{
	...
	public InteractFrame(String filePathIn){
		super();
		filePath = filePathIn;
	...
	public void addPic(int x, int y, String path, Graphics g){
		...
		path = filePath + (filePath.charAt(filePath.length()-1) == '/' ? "" : "/") + path;
		Image img = tk.getImage(path.charAt(path.length()-1) != '/' ? path : new File(path).listFiles()[0].getAbsolutePath());
