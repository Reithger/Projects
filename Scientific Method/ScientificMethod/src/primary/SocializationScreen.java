...
public class SocializationScreen extends InteractFrame{
	...
	public SocializationScreen(String title, JFrame parentFrame, Object paused, Player play, String filePath){
		super(filePath);
		filePathIn = filePath;
		...
		sci.alterRespect(0);
		try{
		  map = ImageIO.read(new File(filePath + "Assets/Backgrounds/GameplayScreen/").listFiles()[0]);
		...
		socialSegment = new Social(this, map, filePath);
		labSegment = new Lab(map, sci.getMaxRespect(), sci.getAssistants(), this, filePathIn);
	...
	public void paintComponent(Graphics g){
		...
		addPicScaled(SCREEN_WIDTH/2, SCREEN_HEIGHT/2, "Assets/Backgrounds/GameplayScreen/", g, 4);
	...
	private void drawSocial(Graphics g){
		...
		if(Math.sqrt(Math.pow(sci.getX() - labStartX, 2) + Math.pow(sci.getY() + sci.getHeight()/2 - labStartY, 2)) < DISTANCE_INTERACT){
			  addPic(sci.getX() + 25, sci.getY() - 25, "Assets/UI/LetterFrame/", g);
			  addPicScaled(sci.getX() + 25, sci.getY() - 25, "Assets/Letters/Alphabet/e.png", g, 3);
		  }
	}
	...
	private void drawLabOutTransition(Graphics g){
		...
		addPicScaled(SCREEN_WIDTH/2, SCREEN_HEIGHT/2, "Assets/UI/LetterFrame/", g, scale > 8 ? scale : 8);
		for(int i = 0; i < outcome.length; i++){
		  for(int j = 0; j < outcome[i].length(); j++){
			String directory = "";
			if(digits.indexOf(outcome[i].charAt(j)+"") == -1){
				directory = "Assets/Letters/Alphabet/" + outcome[i].charAt(j) + ".png";
			}
			else if(digits.indexOf(outcome[i].charAt(j)+"") == 10){
				directory = "Assets/Letters/SpecialCharacters/colon.png";
			}
			else if(digits.indexOf(outcome[i].charAt(j)+"") == 11){
				directory = "Assets/Letters/SpecialCharacters/period.png";
			}
			else{
				directory = "Assets/Letters/Digits/" + outcome[i].charAt(j) + ".png";
			}
	...
	private void drawScore(Graphics g){
		...
		addPicScaled(SCREEN_WIDTH/2, SCREEN_HEIGHT/2, "Assets/UI/LetterFrame/", g, 16);
	...
	private void drawLabStationTiles(Graphics g){
		for(int i = 0; i < labSegment.getStations().size(); i++){
			  ...
			  if(stat.getStateOnOff()){
				  ...
				  stat.getVisual().setVisual(filePathIn + "Assets/UI/Tile/");
			  }
		  }
	}
	...
	private void drawLabActiveStation(Graphics g){
		  if(labSegment.getIndexActive() != -1 && !labSegment.getCurrentLetter().equals(" ")){
			  ...
			  for(int i = 0; i < stat.getDifficulty(); i++){
				Entity frame = new Entity(stat.getVisual().getX() + (int)(25 * Math.cos(2 * Math.PI  * i / (double)stat.getDifficulty())), stat.getVisual().getY() + (int)(25 * Math.sin(2 * Math.PI * i / (double)stat.getDifficulty())), filePathIn + "Assets/UI/LetterFrame/");
				if(stat.getCombo() == 0)
				...
				if(i == labSegment.getCycle())
			      addPicScaled(stat.getVisual().getX() + (int)(25 * Math.cos(2 * Math.PI * i / (double)stat.getDifficulty())), stat.getVisual().getY() + (int)(25 * Math.sin(2 * Math.PI * i / (double)stat.getDifficulty())), "Assets/Letters/Alphabet/" + labSegment.getCurrentLetter() + ".png", g, 3);
			  }
		  }
	...
	private void drawLabStationsRemaining(Graphics g){
		addPicScaled(labSegment.getX(), labSegment.getY(), "Assets/UI/LetterFrame/", g, 2);
		int repre = labSegment.getStationsRemaining();
		String transfer = repre + "";
		for(int i = 0; i < transfer.length(); i++){
			addPicScaled(labSegment.getX() + (i - transfer.length()/2) * 3 * 5, labSegment.getY(), "Assets/Letters/Digits/" + transfer.charAt(i) + ".png", g, 3);
		}
	}
	...
	private void drawGeneric(Graphics g){
		dean.drawToScreen(g);
		int scale_size = 3;
		addPicScaled(dean.getX() + 215, dean.getY() - 15, "Assets/UI/TitleFrame/", g, scale_size+1);
	...
	public void assignDean(Character den){
		dean = new Character(socialSegment.getPortraitY(), socialSegment.getPortraitX(), den.getName(), den.getTitle(), filePathIn + "Assets/Entities/Female/", filePathIn);
