...
public class SocializationScreen extends InteractFrame{
	...
	public SocializationScreen(String title, JFrame parentFrame, Object paused, Player play){
		...
		sci.alterRespect(0);
		try{
		  map = ImageIO.read(new File((new File(InteractFrame.class.getClassLoader().getResource("Assets/Backgrounds/GameplayScreen/").toURI()).listFiles()[0]).getAbsolutePath()));
	...
	private void drawLabStationTiles(Graphics g){
		for(int i = 0; i < labSegment.getStations().size(); i++){
			  ...
			  if(stat.getStateOnOff()){
				  ...
				  stat.getVisual().setVisual("Assets/UI/Tile/");
			  }
		  }
	}
	...
	private void drawLabActiveStation(Graphics g){
		  if(labSegment.getIndexActive() != -1 && !labSegment.getCurrentLetter().equals(" ")){
			  ...
			  for(int i = 0; i < stat.getDifficulty(); i++){
				Entity frame = new Entity(stat.getVisual().getX() + (int)(25 * Math.cos(2 * Math.PI  * i / (double)stat.getDifficulty())), stat.getVisual().getY() + (int)(25 * Math.sin(2 * Math.PI * i / (double)stat.getDifficulty())), "Assets/UI/LetterFrame/");
				if(stat.getCombo() == 0)
	...
	public void assignDean(Character den){
		dean = new Character(socialSegment.getPortraitY(), socialSegment.getPortraitX(), den.getName(), den.getTitle(), "Assets/Entities/Female/");
